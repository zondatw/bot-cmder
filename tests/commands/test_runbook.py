...
async def test_list_lists_only_executable_files(tmp_path: Path):
    ...
    h = reg.get_router("runbook").get_subcommand("list").handler
...
async def test_run_unknown_name(tmp_path: Path):
    reg = _install(FakeConnector())
    h = reg.get_router("runbook").get_subcommand("run").handler
...
async def test_run_with_relative_config_dir_passes_absolute_path_to_subprocess(tmp_path: Path, monkeypatch):
    ...
    fake = FakeConnector()
    reg = _install(fake)
    await reg.get_router("runbook").get_subcommand("run").handler(ctx, ["hello", "world"])
...
def test_runbook_run_is_privileged():
    reg = _install(FakeConnector())
    cmd_run = reg.get_router("runbook").get_subcommand("run")
    cmd_list = reg.get_router("runbook").get_subcommand("list")
